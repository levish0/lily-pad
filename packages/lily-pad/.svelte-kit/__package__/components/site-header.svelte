<script lang="ts">
	...
	function switchLocale(l: string) {
		goto(runtime.localizeHref(runtime.deLocalizeHref(page.url.pathname), { locale: l }));
	}

	function isActive(href: string) {
		const path = runtime.deLocalizeHref(page.url.pathname);
		if (href === '/') return path === '/';
	...
	<div class="mx-auto flex h-14 max-w-[88rem] items-center gap-2 px-6 md:gap-6 lg:px-8">
		...
		<nav class="hidden items-center gap-1 md:flex">
			{#each site.nav as { label, href } (href)}
				<a
					href={runtime.localizeHref(href, { locale })}
				...
				>
					{localized(label, locale, runtime.baseLocale)}
				</a>
			{/each}
		</nav>
