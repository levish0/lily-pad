<script lang="ts">
	...
	const sections = $derived(lp.nav.sections(locale));

	afterNavigate(() => (open = false));

	function isActive(href: string) {
		return (
			runtime.deLocalizeHref(page.url.pathname).replace(/\/$/, '') === (href === '/' ? '/' : href)
		);
	}
		...
		<nav class="mx-auto flex max-w-6xl flex-col gap-6">
			<div class="flex flex-col gap-1">
				{#each site.nav as item (item.href)}
					<a
						href={runtime.localizeHref(item.href, { locale })}
					...
					>
						{localized(item.label, locale, runtime.baseLocale)}
					</a>
				{/each}
			</div>
